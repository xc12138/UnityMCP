...
用于高效使用 Unity-MCP 的常见工作流与实践模式。

## 目录

- [轻流程与升级规则](#轻流程与升级规则)
- [设置与校验](#setup--verification)
- [场景创建工作流](#scene-creation-workflows)
- [脚本开发工作流](#script-development-workflows)
- [资源管理工作流](#asset-management-workflows)
- [测试工作流](#testing-workflows)
- [调试工作流](#debugging-workflows)
- [UI 创建工作流](#ui-creation-workflows)
- [相机与 Cinemachine 工作流](#camera--cinemachine-workflows)
- [ProBuilder 工作流](#probuilder-workflows)
- [图形与渲染工作流](#graphics--rendering-workflows)
- [包管理工作流](#package-management-workflows)
- [包部署工作流](#package-deployment-workflows)
- [API 校验工作流](#api-verification-workflows)
...
本文件是工作流模板库，不代表每次任务都要完整执行全部步骤。

默认先走轻流程：

```text
1. 最小发现   → 只读取一个必要资源，或直接定位目标
2. 执行变更   → 优先使用高层工具；能批量就批量
3. 单通道验证 → 编译类看 console；视觉类看 screenshot；结构类回读资源
4. 失败再升级 → 仅在异常、歧义或高风险条件下补做检查
```
...
遇到以下任一情况，再补做更完整的检查与验证：
- 工具返回 `busy`
- `is_compiling` 为 `true`
- `is_domain_reload_pending` 为 `true`
- 命令无声失败
- 目标对象不明确，或有多个同名对象
...
- 读取 `mcpforunity://instances`
- `find_gameobjects`
- `read_console`
- `get_sha`
- 读取对应 scene / asset / component 资源
...
- 删除脚本、资源、对象
- 包源、registry、Git URL 相关修改
- 大规模批量覆盖现有层级

执行前应先确认范围、影响和是否允许覆盖现有内容。

### 防循环护栏

出现异常时，不要进入无限排障循环。默认遵守以下上限：
- 同一种失败最多重试 2 次
- `editor/state` 最多轮询 3 次
- 连续 2 次出现相同或等价错误时停止自动修复
- 单轮排障只做一种高成本验证：截图、全量资源读取、批量搜索三者选一
...
首次连接、刚恢复连接，或环境明显异常时再做这组检查；不要在每个小任务前机械重复。
...
仅当工具报错、编辑器繁忙、域重载中，或你怀疑当前状态不可靠时，才运行这个检查。
...
**生成场景时始终先调用 `manage_scene(action="create")`**，以获得干净的空场景。这样可避免与现有默认对象（Camera、Light）冲突，防止执行计划创建同名对象时出现 "already exists" 错误。
...
在创建脚本并挂载组件后，使用 `set_property` 绑定 GameObject 间的交叉引用。可使用 `{"name": "ObjectName"}` 格式按名称引用场景对象：
...
当脚本使用 `OnTriggerEnter` / `OnTriggerStay` / `OnTriggerExit` 时，发生碰撞的两个对象中**至少一个**必须带有 `Rigidbody` 组件。常见模式如下：
...
### 使用 `manage_script(action="update")` 覆盖脚本

当生成的脚本需要进行重写（例如，为了添加自动布线逻辑时），请使用“更新”操作，而非删除后再重新创建：
...
### 创建 EventSystem（每个场景的 UI 交互至少需要一个）
...
### 添加 Layout Group（Vertical/Horizontal/Grid）
...
### 通过 Renderer Features（URP）添加全屏效果
