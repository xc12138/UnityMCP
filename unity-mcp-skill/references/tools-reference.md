...
| `textmeshpro` | bool | 是否可用 TextMesh Pro（`TMP_Text`、`TMP_InputField`）。在 UGUI 2.x 中该能力可由 UGUI 自带，不一定对应独立 `com.unity.textmeshpro` UPM 包。 |
...
- **文本**：若 `textmeshpro` 为 true，按“可用 TMP 能力”处理并优先使用 `TextMeshProUGUI`，而不是旧版 `Text`。
