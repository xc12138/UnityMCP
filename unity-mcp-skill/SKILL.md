...
| **UI** | `manage_ui`, `batch_execute` with `manage_gameobject` + `manage_components` | **UI Toolkit**：用 `manage_ui` 创建 UXML/USS、挂载 UIDocument、检查可视树。**uGUI (Canvas)**：用 `batch_execute` 组合创建 Canvas、Panel、Button、Text、Slider、Toggle、Input Field。**先读 `mcpforunity://project/info` 判断 uGUI/TMP/Input System/UI Toolkit 可用性。**（见 [UI workflows](references/workflows.md#ui-creation-workflows)） |
